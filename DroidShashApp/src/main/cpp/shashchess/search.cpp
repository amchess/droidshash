...
void Thread::search() {
    ...
    bool  possibleMCTSByValue =
      ((rootPos.this_thread()->shashinWinProbabilityRange == SHASHIN_POSITION_HIGH_PETROSIAN)
       || (rootPos.this_thread()->shashinWinProbabilityRange == SHASHIN_POSITION_MIDDLE_HIGH_PETROSIAN)
       || (rootPos.this_thread()->shashinWinProbabilityRange == SHASHIN_POSITION_MIDDLE_PETROSIAN));
    if (!mainThread && bool(Options["MCTS by Shashin"]) && multiPV == 1 && !maybeDraw
        && possibleMCTSByValue && idx <= (size_t) (mctsThreads)
        && !Utility::is_game_decided(rootPos, rootPosValue))
    {
...
Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        || (pos.this_thread()->shashinWinProbabilityRange != SHASHIN_POSITION_HIGH_TAL))
    {
        ...
        if ((ss - 1)->statScore < 17329 && eval >= beta && eval >= ss->staticEval
            && ss->staticEval >= beta - 21 * depth - improvement / 13 + 258
            && pos.non_pawn_material(us) && !disableNMAndPC  //Kelly
            && (((isShashinHighMiddle(pos) || (!isShashinPositionTal(pos))) && !PvNode
                 && (ss - 1)->currentMove != MOVE_NULL && !excludedMove
                 && (ss->ply >= thisThread->nmpMinPly))
                || (((!(isShashinHighMiddle(pos))) && isShashinPositionTal(pos))
                    && !thisThread->nmpGuard && !gameCycle && beta < VALUE_MATE_IN_MAX_PLY
                    && !kingDanger
        ...
        if (PvNode && !ttMove
            && ((!gameCycle && depth >= 3 && (ss - 1)->moveCount > 1)
                || ((pos.this_thread()->shashinWinProbabilityRange
                     != SHASHIN_POSITION_HIGH_TAL))))  //from Crystal yes HT C No MLT TC MHP HP
        {
            depth -=
              2 + (pos.this_thread()->shashinWinProbabilityRange != SHASHIN_POSITION_HIGH_TAL)
            ...
            while ((move = mp.next_move()) != MOVE_NONE)
                if (move != excludedMove && pos.legal(move))
                {
                    ...
                    ss->continuationHistory =
                      &thisThread->continuationHistory[ss->inCheck][true][pos.moved_piece(move)]
                                                      [to_sq(move)];
                    pos.do_move(move, st);
...
moves_loop:  // When in check, search starts here
    ...
    if (ss->inCheck && !PvNode
        ...
        //from Crystal end
        && (tte->bound() & BOUND_LOWER) && tte->depth() >= depth - 4 && ttValue >= probCutBeta
        && abs(ttValue) <= VALUE_KNOWN_WIN && abs(beta) <= VALUE_KNOWN_WIN)
        return probCutBeta;
    ...
    bool allowLMR = depth > 1 && !gameCycle && (!PvNode || ss->ply > 1);
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
        ...
        )
        {
                ...
                || (pos.this_thread()->shashinWinProbabilityRange != SHASHIN_POSITION_HIGH_TAL))
            {
                ...
                if (capture || givesCheck)
                {
                    ...
                    if (!pos.see_ge(move, occupied, Value(-205) * depth))
                    {
                        ...
                        while (leftEnemies && !attacks)
                        {
                            ...
                            if (attacks
                                && (sq != pos.square<KING>(~us)
                                    && (pos.attackers_to(sq, pos.pieces()) & pos.pieces(us))))
                                attacks = 0;
                        }
                        if (!attacks)
                            continue;
                ...
                else
                {
                    ...
                    if (
                      !ss->inCheck && lmrDepth < 12
                      && ((history < 20500 - 3875 * (depth - 1))
                          || ((pos.this_thread()->shashinWinProbabilityRange
                               != SHASHIN_POSITION_HIGH_TAL)
                              && (pos.this_thread()->shashinWinProbabilityRange
                                  != SHASHIN_POSITION_CAPABLANCA_PETROSIAN)))  //from Crystal yes HT C CP No MHP
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode
                ...
                && abs(ttValue) < VALUE_KNOWN_WIN && (tte->bound() & BOUND_LOWER)
                && tte->depth() >= depth - 3)
            {
...
skipExtensionAndPruning:  // full threads search patch
        ...
        pos.do_move(move, st, givesCheck);
        bool lateKingDanger =
          (rootDepth > 10 && ourMove && ss->ply < 7 && pos.king_danger());  //from Crystal
        ...
        if (PvNode)
        {
            r -=
              ...
              + (((pos.this_thread()->shashinWinProbabilityRange != SHASHIN_POSITION_CAPABLANCA_TAL)
                  ...
                  && (pos.this_thread()->shashinWinProbabilityRange != SHASHIN_POSITION_HIGH_TAL))
                   ? (12 / (3 + depth))
                   : 1);  //from Crystal Yes HT CCT C CCP No MP MHP
        }
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][from_to(move)]
                      ...
                      + (*contHist[1])[movedPiece][to_sq(move)]
                      + (*contHist[3])[movedPiece][to_sq(move)] - 4006;
    ...
    */
    ...
    {
        int bonus = (depth > 5) + (PvNode || cutNode) + (bestValue < alpha - 113 * depth)
                  + ((ss - 1)->moveCount > 12);
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
