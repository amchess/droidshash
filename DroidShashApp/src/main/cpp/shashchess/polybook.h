...
public:
    ...
    ShashChess::Move probe(ShashChess::Position& pos, bool bestBookMove);

private:

    ShashChess::Key polyglot_key(const ShashChess::Position& pos);
    ShashChess::Move pg_move_to_sf_move(const ShashChess::Position & pos, unsigned short pg_move);

    int find_first_key(uint64_t key);
    int get_key_data();

    bool check_draw(ShashChess::Position& pos, ShashChess::Move m);
