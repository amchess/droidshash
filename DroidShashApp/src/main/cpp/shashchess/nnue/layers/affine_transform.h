...
class AffineTransform<
  InDims,
  OutDims,
  std::enable_if_t<(ceil_to_multiple<IndexType>(InDims, MaxSimdWidth) >= LargeInputSize)>> {
   public:
    // Input/output type
    ...
    static_assert(
      PaddedInputDimensions >= LargeInputSize,
      "Something went wrong. This specialization (for large inputs) should not have been chosen.");
    ...
    */
    static IndexType get_weight_index(IndexType i) {
        ...
        const IndexType idx = bigBlock * BigBlockSize
                            + smallBlockRow * SmallBlockSize * NumOutputRegs
                            + smallBlockCol * SmallBlockSize + rest;
    ...
    #define vec_haddx4 Simd::neon_m128_haddx4
        ...
        for (IndexType bigBlock = 0; bigBlock < NumBigBlocks; ++bigBlock)
        {
            ...
            {
                const weight_vec_t* weightvec = reinterpret_cast<const weight_vec_t*>(
                  weights + bigBlock * BigBlockSize + smallBlock * SmallBlockSize * NumOutputRegs);

                const in_vec_t in0 = invec[smallBlock + 0];
                const in_vec_t in1 = invec[smallBlock + 1];

                for (IndexType k = 0; k < NumOutputRegs; ++k)
                    vec_add_dpbusd_32x2(acc[k], in0, weightvec[k], in1,
                                        weightvec[k + NumOutputRegs]);
            }
            ...
            if constexpr (NumOutputRegs % 4 == 0)
            {
                ...
                for (IndexType k = 0; k < NumOutputRegs; k += 4)
                {
                    ...
                    outputvec[idx] =
                      vec_haddx4(acc[k + 0], acc[k + 1], acc[k + 2], acc[k + 3], biasvec[idx]);
                }
            }
            else
            {
...
class AffineTransform<
  ...
  std::enable_if_t<(ceil_to_multiple<IndexType>(InDims, MaxSimdWidth) < LargeInputSize)>> {
   public:
    ...
    static_assert(
      PaddedInputDimensions < LargeInputSize,
      "Something went wrong. This specialization (for small inputs) should not have been chosen.");
