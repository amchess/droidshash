...
static void on_persisted_learning(const Option& o) {
    if (!(o == "Off"))
    {
        LD.set_learning_mode(o);
    }
}  //Kelly learning
static void on_readonly_learning(const Option& o) {
    if (!(o == "Off"))
    {
        LD.set_readonly(o);
    }
...
#endif
    //livebook end
    o["Full depth threads"] << Option(
      0, 0, 512, on_full_threads);  //if this is used, must be after #Threads is set.
    o["Opening variety"] << Option(0, 0, 40);
    o["Persisted learning"] << Option("Off var Off var Standard var Self", "Off",
                                      on_persisted_learning);
