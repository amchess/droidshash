/*
    ...
    Copyright (C) 2013  Peter Österlund, amchesssterlund2@gmail.com
...
import org.amchess.droidshash.activities.util.FENFile;
import org.amchess.droidshash.activities.util.FENFile.FenInfo;
import org.amchess.droidshash.activities.util.FENFile.FenInfoResult;
import org.amchess.droidshash.databinding.LoadFenBinding;
...
public class LoadFEN extends ListActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if ("org.amchess.droidshash.loadFen".equals(action)) {
            ...
            workThread.start();
        } else if ("org.amchess.droidshash.loadNextFen".equals(action) ||
                   "org.amchess.droidshash.loadPrevFen".equals(action)) {
            fenFile = new FENFile(fileName);
            boolean next = action.equals("org.amchess.droidshash.loadNextFen");
            final int loadItem = defaultItem + (next ? 1 : -1);
            if (loadItem < 0) {
                DroidShashApp.toast(R.string.no_prev_fen, Toast.LENGTH_SHORT);
                ...
                workThread = new Thread(() -> {
                    ...
                    runOnUiThread(() -> {
                        if (loadItem >= fensInFile.size()) {
                            DroidShashApp.toast(R.string.no_next_fen, Toast.LENGTH_SHORT);
                            setResult(RESULT_CANCELED);
                            finish();
                        } else {
    ...
    private boolean readFile() {
        ...
        if (p.first != FenInfoResult.OK) {
            ...
            if (p.first == FenInfoResult.OUT_OF_MEMORY) {
                runOnUiThread(() -> DroidShashApp.toast(R.string.file_too_large, Toast.LENGTH_SHORT));
            }
            setResult(RESULT_CANCELED);
            finish();
            return false;
    ...
    private void sendBackResult(FenInfo fi, boolean toast) {
        ...
        if (fen != null) {
            if (toast)
                DroidShashApp.toast(String.valueOf(fi.gameNo) + ": " + fen, Toast.LENGTH_SHORT);
